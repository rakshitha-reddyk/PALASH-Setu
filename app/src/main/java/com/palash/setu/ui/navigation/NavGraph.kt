...
fun PalashNavGraph(
    ...
    dictionaryDao: FLNDictionaryDao,
    ...
    NavHost(navController, startDestination = start) {
        composable("onboarding") {
            OnboardingScreen(preferences, userDao) {
                navController.navigate("live-translator") {
                    popUpTo("onboarding") { inclusive = true }
                }
            }
        }
        composable("live-translator") {
            MainTabLayout(navController, tabs) {
                LiveTranslatorScreen(preferences.targetLanguage, dictionaryDao)
            }
        }
        composable("fln-scripts") {
            MainTabLayout(navController, tabs) {
                FLNScriptsScreen(preferences.targetLanguage)
            }
        }
        composable("worksheet-generator") {
            MainTabLayout(navController, tabs) {
                WorksheetScreen(preferences.teacherId, preferences.targetLanguage, worksheetDao)
            }
        }
    }
}
